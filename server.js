...
.ctrl-btn{
  ...
  color:white;
  font-size:18px;
  touch-action:none; /* مهم لمنع long-press menu */
}
...
<div class="grid">
<div></div>
<button class="ctrl-btn" id="btnF" data-cmd="FWD">⬆</button>
<div></div>

<button class="ctrl-btn" id="btnL" data-cmd="LEFT">⬅</button>
<button class="ctrl-btn" onclick="send('STOP')">⛔</button>
<button class="ctrl-btn" id="btnR" data-cmd="RIGHT">➡</button>

<div></div>
<button class="ctrl-btn" id="btnB" data-cmd="BACK">⬇</button>
<div></div>
</div>
...
function bindHold(btn){
  ...
  const start = (e)=>{
    ...
    send(cmd);
    holdTimer = setInterval(()=>send(cmd),120); // أسرع استجابة
  };
...
document.getElementById("status").innerHTML = danger ? "⚠️ DANGER" : "✅ SAFE";
